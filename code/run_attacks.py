...
def save_flow_imgs(
    flow, flowdir, visflow, dataset_name, traj_name, save_dir_suffix="_clean"
):
...
def save_poses_se(
    ...
    if plot_est:
        ax.scatter(
            poses[:, 0, -1], poses[:, 1, -1], poses[:, 2, -1], s=15, color=(1, 0, 1)
        )
    if plot_gt:
        ax.scatter(
            gt_poses[:, 0, -1],
    ...
    for p_idx, (p_gt, p) in enumerate(zip(gt_poses, poses)):
        ...
        if p_idx % factor == 0:
            if plot_est:
                ax.plot(
                    [p[0, -1], p[0, -1] + sc * xlims * p[0, 0]],
                    [p[1, -1], p[1, -1] + sc * ylims * p[1, 0]],
                    zs=[p[2, -1], p[2, -1] + sc * zlims * p[2, 0]],
                    color=(1, 0, 1),
                )
                ax.plot(
                    [p[0, -1], p[0, -1] + sc * xlims * p[0, 1]],
                    [p[1, -1], p[1, -1] + sc * ylims * p[1, 1]],
                    zs=[p[2, -1], p[2, -1] + sc * zlims * p[2, 1]],
                    color=(0, 1, 1),
                )
                ax.plot(
                    [p[0, -1], p[0, -1] + sc * xlims * p[0, 2]],
                    [p[1, -1], p[1, -1] + sc * ylims * p[1, 2]],
                    zs=[p[2, -1], p[2, -1] + sc * zlims * p[2, 2]],
                    color=(0.5, 0.5, 0.5),
                )
            if plot_gt:
                ax.plot(
                    [p_gt[0, -1], p_gt[0, -1] + sc * xlims * p_gt[0, 0]],
                    [p_gt[1, -1], p_gt[1, -1] + sc * ylims * p_gt[1, 0]],
                    zs=[p_gt[2, -1], p_gt[2, -1] + sc * zlims * p_gt[2, 0]],
                    color=(1, 0, 0, alpha),
                )
                ax.plot(
                    [p_gt[0, -1], p_gt[0, -1] + sc * xlims * p_gt[0, 1]],
                    [p_gt[1, -1], p_gt[1, -1] + sc * ylims * p_gt[1, 1]],
                    zs=[p_gt[2, -1], p_gt[2, -1] + sc * zlims * p_gt[2, 1]],
                    color=(0, 1, 0, alpha),
                )
                ax.plot(
                    [p_gt[0, -1], p_gt[0, -1] + sc * xlims * p_gt[0, 2]],
                    [p_gt[1, -1], p_gt[1, -1] + sc * ylims * p_gt[1, 2]],
                    zs=[p_gt[2, -1], p_gt[2, -1] + sc * zlims * p_gt[2, 2]],
                    color=(0, 0, 1, alpha),
                )
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
...
def save_unprocessed_imgs(
    img_dir, dataset_name, traj_name, img1_I0, img2_I0, img1_I1, img2_I1
):
...
def save_adv_imgs(
    adv_img_dir, adv_pert_dir, dataset_name, traj_name, img1_adv, img2_adv, best_pert
):
...
def test_model(
    ...
    if window_size is None:
        if device is None:
            ...
            crit_results = [
                crit((motions, flow), scale_gt, motions_target, target_pose)
                .detach()
                .cpu()
        ...
        crit_results = [
            crit_result_device.clone().detach().cpu()
            for crit_result_device in crit_results_device
        ]
    ...
    if device is None:
        ...
        crit_results = [
            crit((motions, flow), scale_gt, motions_target, target_pose).detach().cpu()
            for crit in criterions
        ]
...
def test_clean_multi_inputs(args):
    ...
    frames_clean_criterions_list = [
        [[] for i in range(args.traj_len)] for crit in args.criterions
    ]
    ...
    for traj_idx, traj_data in enumerate(args.testDataloader):
        ...
        with torch.no_grad():
            (motions, flow), crit_results = test_model(
                args.model,
                args.criterions,
                img1_I0,
                img2_I0,
            ...
            for crit_idx, crit_result in enumerate(crit_results):
                ...
                for frame_idx, frame_clean_crit in enumerate(crit_result_list):
                    frames_clean_criterions_list[crit_idx][frame_idx].append(
                        frame_clean_crit
                    )
            del crit_results
    ...
    for crit_idx, frames_clean_crit_list in enumerate(frames_clean_criterions_list):
        frames_clean_crit_mean = [
            np.mean(crit_list) for crit_list in frames_clean_crit_list
        ]
        frames_clean_crit_std = [
            np.std(crit_list) for crit_list in frames_clean_crit_list
        ]
        print(
            "frames_clean_"
            + args.criterions_names[crit_idx]
            + "_mean over all ("
            + str(len(traj_name_list))
            + ") trajectories:"
        )
    ...
    print(
        "traj_mask_l0_ratio_mean over all ("
        + str(len(traj_name_list))
        + ") trajectories:"
    )
    print(traj_mask_l0_ratio_mean)
    print(
        "traj_mask_l0_ratio_std over all ("
        + str(len(traj_name_list))
        + ") trajectories:"
    )
    print(traj_mask_l0_ratio_std)

    frames_motions_scales = [
        [motions_scales[i] for motions_scales in traj_motions_scales]
        for i in range(traj_len - 1)
    ]
    frames_motions_scales_means = [
        np.mean(scale_list) for scale_list in frames_motions_scales
    ]
    frames_motions_scales_stds = [
        np.std(scale_list) for scale_list in frames_motions_scales
    ]
    frames_mean_dist = [0]
    curr_dist = 0
    ...
    print(
        "frames_motions_scales_means over all ("
        + str(len(traj_name_list))
        + ") trajectories:"
    )
    print(frames_motions_scales_means)
    print(
        "frames_motions_scales_stds over all ("
        + str(len(traj_name_list))
        + ") trajectories:"
    )
...
def test_adv_trajectories(
    ...
    for traj_idx, traj_data in enumerate(dataloader):
        ...
        with torch.no_grad():
            img1_adv, img2_adv = attack.apply_pert(
                traj_pert,
                img1_I0,
                img2_I0,
                img1_delta,
            ...
            (motions_adv, flow_adv), crit_results = test_model(
                model,
                criterions,
                img1_adv,
                img2_adv,
...
def report_adv_deviation(
    dataset_idx_list,
    dataset_name_list,
    traj_name_list,
    traj_indices,
    traj_clean_crit_list,
    traj_adv_crit_list,
    save_csv,
    output_dir,
    crit_str,
    ...
    if save_csv:
        ...
        with open(csv_path, "w") as f:
            ...
            writer.writeheader()
            for traj_idx, traj_name in enumerate(traj_name_list):
                for frame_idx, frame_clean_crit in enumerate(
                    traj_clean_crit_list[traj_idx]
                ):
                    data = {
                        "dataset_idx": dataset_idx_list[traj_idx],
                        ...
                        "adv_delta_"
                        + crit_str: traj_delta_crit_list[traj_idx][frame_idx],
                        "adv_ratio_"
                        + crit_str: traj_ratio_crit_list[traj_idx][frame_idx],
                        "adv_delta_ratio_"
                        + crit_str: traj_adv_crit_list[traj_idx][frame_idx],
                    }
    ...
    frames_delta_crit_mean = [
        np.mean(crit_list) for crit_list in frames_delta_crit_list
    ]
    frames_delta_crit_std = [np.std(crit_list) for crit_list in frames_delta_crit_list]
    frames_ratio_crit_mean = [
        np.mean(crit_list) for crit_list in frames_ratio_crit_list
    ]
    frames_ratio_crit_std = [np.std(crit_list) for crit_list in frames_ratio_crit_list]
    frames_delta_ratio_crit_mean = [
        np.mean(crit_list) for crit_list in frames_delta_ratio_crit_list
    ]
    frames_delta_ratio_crit_std = [
        np.std(crit_list) for crit_list in frames_delta_ratio_crit_list
    ]
...
def run_attacks_train(args):
    ...
    best_pert, clean_loss_list, all_loss_list, all_best_loss_list = attack.perturb(
        args.testDataloader, motions_target_list, eps=args.eps, device=args.device
    )
    ...
    traj_adv_criterions_list = test_adv_trajectories(
        args.testDataloader,
        args.model,
        motions_target_list,
        attack,
    ...
    (
        traj_adv_rms_list,
        traj_adv_mean_partial_rms_list,
        traj_adv_target_rms_list,
        traj_adv_target_mean_partial_rms_list,
    ) = tuple(traj_adv_criterions_list)
...
def report_mean_results(
    ...
    folds_frames_clean_crit_mean = [
        np.mean(rms_list) for rms_list in folds_frames_clean_crit_means
    ]
    folds_frames_clean_crit_std = [
        np.mean(rms_list) for rms_list in folds_frames_clean_crit_stds
    ]
    folds_frames_adv_crit_mean = [
        np.mean(rms_list) for rms_list in folds_frames_adv_crit_means
    ]
    folds_frames_adv_crit_std = [
        np.mean(rms_list) for rms_list in folds_frames_adv_crit_stds
    ]
    folds_frames_delta_crit_mean = [
        np.mean(rms_list) for rms_list in folds_frames_delta_crit_means
    ]
    folds_frames_delta_crit_std = [
        np.mean(rms_list) for rms_list in folds_frames_delta_crit_stds
    ]
    folds_frames_ratio_crit_mean = [
        np.mean(rms_list) for rms_list in folds_frames_ratio_crit_means
    ]
    folds_frames_ratio_crit_std = [
        np.mean(rms_list) for rms_list in folds_frames_ratio_crit_stds
    ]
    folds_frames_delta_ratio_crit_mean = [
        np.mean(rms_list) for rms_list in folds_frames_delta_ratio_crit_means
    ]
    folds_frames_delta_ratio_crit_std = [
        np.mean(rms_list) for rms_list in folds_frames_delta_ratio_crit_stds
    ]
...
def run_attacks_open_loop_opt(args):
    ...
    for eval_fold_idx in eval_folds_indices_list:
        ...
        eval_indices = [
            ...
            if dataset_idx_list[traj_idx] == eval_fold_idx
        ]
        eval_dataset = args.dataset_class(
            args.test_dir,
        ...
        (
            ...
            train_traj_names,
            train_indices,
            traj_clean_train_target_mean_partial_rms_list,
            traj_adv_train_target_mean_partial_rms_list,
            args.save_csv,
            train_output_dir,
            crit_str="target_mean_partial_rms",
        ...
        (
            ...
            train_traj_names,
            train_indices,
            traj_clean_train_target_rms_list,
            traj_adv_train_target_rms_list,
            args.save_csv,
            train_output_dir,
            crit_str="target_rms",
        ...
        (
            ...
            train_traj_names,
            train_indices,
            traj_clean_train_mean_partial_rms_list,
            traj_adv_train_mean_partial_rms_list,
            args.save_csv,
            train_output_dir,
            crit_str="mean_partial_rms",
        ...
        (
            ...
            eval_traj_names,
            eval_indices,
            traj_clean_eval_target_mean_partial_rms_list,
            traj_adv_eval_target_mean_partial_rms_list,
            args.save_csv,
            eval_output_dir,
            crit_str="target_mean_partial_rms",
        ...
        (
            ...
            eval_traj_names,
            eval_indices,
            traj_clean_eval_target_rms_list,
            traj_adv_eval_target_rms_list,
            args.save_csv,
            eval_output_dir,
            crit_str="target_rms",
        ...
        (
            ...
            eval_traj_names,
            eval_indices,
            traj_clean_eval_mean_partial_rms_list,
            traj_adv_eval_mean_partial_rms_list,
            args.save_csv,
            eval_output_dir,
            crit_str="mean_partial_rms",
...
def test_clean(args):
    print(
        "Testing the visual odometer on the I1 albedo image compared to the clean I0 albedo image, "
